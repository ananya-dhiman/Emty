...
async fn fetch_sync_state(port: u16, account_id: &str) -> Option<SyncState> {
    let client = reqwest::Client::new();
    let res = client
        .get(format!("http://localhost:{}/api/sync/state/{}", port, account_id))
        .send()
        .await
        .ok()?;
...
pub fn check_on_launch(app_handle: AppHandle, port: u16) {
    tauri::async_runtime::spawn(async move {
        // We use "default" as account ID for now, or fetch from auth logic if needed
        let account_id = "default";
        
        if let Some(state) = fetch_sync_state(port, account_id).await {
            if let Some(last_sync) = state.last_sync_timestamp {
                ...
                if gap_days >= 7 {
                    ...
                    if gap_ms > interval_ms {
                        trigger_sync(port, account_id, "urgent").await;
                    }
                }
            }
        }
        ...
        loop {
            ...
            if let Some(state) = fetch_sync_state(port, account_id).await {
                ...
                if let Some(last_sync) = state.last_sync_timestamp {
                    ...
                    if gap_ms >= interval_ms {
                        trigger_sync(port, account_id, "scheduled").await;
                    }
                }
            }
        }
